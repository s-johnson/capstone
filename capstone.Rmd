...
save(sim.slope.exp, file = paste(data_path, "sim.slope.exp.Rda", sep = ""))
```


```{r}
plot.sim.slope <- ggplot(sim.slope.exp, aes(x = modx.value, y = estimate)) +
  geom_point() +
  geom_errorbar(data = sim.slope.exp, aes(ymin = conf.low, ymax = conf.high)) +
  scale_x_continuous(breaks = 0:11) +
...
```{r}
X.family <- model.matrix(glm.ixn.family.rm)
dof.family <- nrow(X.family) - ncol(X.family)
vcov.family <- vcov(glm.ixn.family.rm)
t.family <- qt(0.975, dof.family)
coef.family <- coef(glm.ixn.family.rm)
...
sim.slope.family <- data.frame( family = factor(c("Unsupportive", 
                                                  ...
                                                  levels = c("Unsupportive",
                                                             ...
                                                             "Supportive")),
                                 estimate = unname(
                                   c(glm.ixn.family.rm$coefficients[2],
                                     glm.ixn.family.rm$coefficients[20] +
                                       glm.ixn.family.rm$coefficients[2],
                                     glm.ixn.family.rm$coefficients[21] +
                                       glm.ixn.family.rm$coefficients[2])),
...
sim.slope.family <- sim.slope.family %>% 
  mutate(conf.low = exp(estimate - t.family*se),
...
```{r}
X.provider <- model.matrix(glm.ixn.provider.rm)
dof.provider <- nrow(X.provider) - ncol(X.provider)
vcov.provider <- vcov(glm.ixn.provider.rm)
t.provider <- qt(0.975, dof.provider)
coef.provider <- coef(glm.ixn.provider.rm)
...
sim.slope.provider <- data.frame( provider = factor(c("No Knowledgeable Provider",
                                                  ...
                                                  levels = c("No Knowledgeable Provider",
                                                             "Has Knowledgeable Provider")),
                                 estimate = unname(
                                   c(glm.ixn.provider.rm$coefficients[2],
                                     glm.ixn.provider.rm$coefficients[19] +
                                       glm.ixn.provider.rm$coefficients[2])),
                                 se = c(sqrt(vcov.provider[2,2]),
                                        sqrt(vcov.provider[2,2] + 
                                               vcov.provider[19,19] + 
                                               vcov.provider[2,19])))
sim.slope.provider <- sim.slope.provider %>% 
  mutate(conf.low = exp(estimate - t.provider*se),
...
```{r}
X.id.name <- model.matrix(glm.ixn.id.name.rm)
dof.id.name <- nrow(X.id.name) - ncol(X.id.name)
vcov.id.name <- vcov(glm.ixn.id.name.rm)
t.id.name <- qt(0.975, dof.id.name)
coef.id.name <- coef(glm.ixn.id.name.rm)
...
sim.slope.id.name <- data.frame( id.name = factor(c("None", "Some", "All"),
                                 ...
                                 estimate = unname(
                                   c(glm.ixn.id.name.rm$coefficients[2],
                                     glm.ixn.id.name.rm$coefficients[20] +
                                       glm.ixn.id.name.rm$coefficients[2],
                                     glm.ixn.id.name.rm$coefficients[21] +
                                       glm.ixn.id.name.rm$coefficients[2])),
...
sim.slope.id.name <- sim.slope.id.name %>% 
  mutate(conf.low = exp(estimate - t.id.name*se),
...
```{r}
X.id.gender <- model.matrix(glm.ixn.id.gender.rm)
dof.id.gender <- nrow(X.id.gender) - ncol(X.id.gender)
vcov.id.gender <- vcov(glm.ixn.id.gender.rm)
t.id.gender <- qt(0.975, dof.id.gender)
coef.id.gender <- coef(glm.ixn.id.gender.rm)
...
sim.slope.id.gender <- data.frame( id.gender = factor(c("None", "Some", "All"),
                                                  levels = c("None", "Some", "All")),
                                 estimate = unname(
                                   c(glm.ixn.id.gender.rm$coefficients[2],
                                     glm.ixn.id.gender.rm$coefficients[20] +
                                       glm.ixn.id.gender.rm$coefficients[2],
                                     glm.ixn.id.gender.rm$coefficients[21] +
                                       glm.ixn.id.gender.rm$coefficients[2])),
...
sim.slope.id.gender <- sim.slope.id.gender %>% 
  mutate(conf.low = exp(estimate - t.id.gender*se),
...
```{r}
# Interaction model - affirm hormone/counseling only - rm
...
```{r}
X.hormone <- model.matrix(glm.ixn.hormone.rm)
dof.hormone <- nrow(X.hormone) - ncol(X.hormone)
vcov.hormone <- vcov(glm.ixn.hormone.rm)
t.hormone <- qt(0.975, dof.hormone)
coef.hormone <- coef(glm.ixn.hormone.rm)
...
sim.slope.hormone <- data.frame( hormone = factor(c("None", "Some", "All"),
                                                  levels = c("None", "Some", "All")),
                                 estimate = unname(
                                   c(glm.ixn.hormone.rm$coefficients[2],
                                     glm.ixn.hormone.rm$coefficients[20] +
                                       glm.ixn.hormone.rm$coefficients[2],
                                     glm.ixn.hormone.rm$coefficients[21] +
                                       glm.ixn.hormone.rm$coefficients[2])),
...
sim.slope.hormone <- sim.slope.hormone %>% 
  mutate(conf.low = exp(estimate - t.hormone*se),
...
```{r}
X.med <- model.matrix(glm.ixn.med.rm)
dof.med <- nrow(X.med) - ncol(X.med)
vcov.med <- vcov(glm.ixn.med.rm)
t.med <- qt(0.975, dof.med)
coef.med <- coef(glm.ixn.med.rm)
...
sim.slope.med <- data.frame( med = factor(c("None", "Some", "All"),
                                                  levels = c("None", "Some", "All")),
                                 estimate = unname(
                                   c(glm.ixn.med.rm$coefficients[2],
                                     glm.ixn.med.rm$coefficients[20] +
                                       glm.ixn.med.rm$coefficients[2],
                                     glm.ixn.med.rm$coefficients[21] +
                                       glm.ixn.med.rm$coefficients[2])),
...
sim.slope.med <- sim.slope.med %>% 
  mutate(conf.low = exp(estimate - t.med*se),
